...
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; // this is the binary semaphore variable for thread synchronization
...
void *calculateColumnSum(void *arg) // arg parameter is used for passing the column index to thread
{
    ...
    pthread_mutex_lock(&mutex); // lock the mutex variable to ensure that only one thread at a time can update the sumOfColumns array
    sumOfColumns[columnIndex] = sum; // thread adds the sum that it calculated to sumOfColumns array and finishes its task
    pthread_mutex_unlock(&mutex); // after updating the array, mutex is unlocked, allowing other threads to access the array
